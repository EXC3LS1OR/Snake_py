...
class SnakeGame:
    def __init__(self, width=400, height=400, fps=80):
        self.width = width
        self.height = height
    ...
    def _place_food(self):
        if(self.score == (self.width/BLOCK_SIZE)*(self.height/BLOCK_SIZE) - 3):
            self._pause()
    ...
    def play_step(self):
        ...
        if(self.head.y%40 == 0 and self.head.x%40 == 0):
            if(math.dist(self.food, Point(self.head.x - BLOCK_SIZE, self.head.y)) < math.dist(self.food, Point(self.head.x, self.head.y + BLOCK_SIZE))):
                self.direction = Direction.LEFT
                if(self._check_collision( self.direction )):
                    self.direction = Direction.DOWN
            elif(math.dist(self.food, Point(self.head.x - BLOCK_SIZE, self.head.y)) == math.dist(self.food, Point(self.head.x, self.head.y + BLOCK_SIZE))):
